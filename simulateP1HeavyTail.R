simulatePlatformOf1HeavyTail = function(PARAM_ARGS, MODEL_ARGS) {
  ...
  if (PARAM_ARGS[["AR1_PHI"]] == 0) {
    Y = ((X %*% BETAS) %>% c()) + stats::rcauchy(nrow(X), 0, scale = MODEL_ARGS[["SIGMA"]])
  } else {
    Y_ar = arima.sim(model = list(ar = PARAM_ARGS[["AR1_PHI"]] / 100), 
                     rand.gen = rcauchy, 
                     scale = MODEL_ARGS[["SIGMA"]],
  ...
  for (period in adaptive_periods) {
    ...
    if (PARAM_ARGS[["AR1_PHI"]]  == 0) {
      Y = (X %*% BETAS) + stats::rcauchy(nrow(X), 0, scale = MODEL_ARGS[["SIGMA"]])
    } else {
      Y_ar = arima.sim(model = list(ar = PARAM_ARGS[["AR1_PHI"]] / 100), 
                       n = PARAM_ARGS[["NUM_OBVS_ADAPTIVE"]],
                       rand.gen = rcauchy, 
                       scale = MODEL_ARGS[["SIGMA"]])
      Y = Y_ar + (X %*% BETAS)
    }
