simulatePlatformOf1 = function(PARAM_ARGS, MODEL_ARGS) {
  ...
  for (period in adaptive_periods) {
    ...
    CLIN_INTERVAL = posterior::as_draws_df(POSTERIOR) %>% 
      ...
      group_by(treatment) %>% 
      summarize(
        in_interval1 = mean(val >= 120 & val <= MODEL_ARGS[["DELTA"]]),
        in_interval2 = mean(val <= MODEL_ARGS[["DELTA"]])
      ) 
    
    CLIN_INTERVAL_ROW = CLIN_INTERVAL %>% 
      pivot_wider(values_from = c("in_interval1", "in_interval2"), 
  ...
  CLIN_INTERVAL_TABLE_WIDE = CLIN_INTERVAL_TABLE %>% 
    select(period, contains("interval")) %>% 
    pivot_wider(names_from = period, 
                values_from = c("in_interval1_b_X1", "in_interval2_b_X1",
                                "in_interval1_b_X2", "in_interval2_b_X2",
                                "in_interval1_b_X3", "in_interval2_b_X3"))
