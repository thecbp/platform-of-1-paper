simulatePlatformOf1Delayed = function(PARAM_ARGS, MODEL_ARGS) {
  ...
  ##############################################################################
  # Pre-adaptive phase I: initialize the new treatment
  ##############################################################################
  
  adaptive_start = (PARAM_ARGS[["NUM_TRTS"]] * PARAM_ARGS[["NUM_INIT_CYCLES"]]) + 1
  adaptive_ph1_periods = adaptive_start:20
  
  for (period in adaptive_ph1_periods) {
  ...
  ##############################################################################
  # Pre-adaptive phase II: initialization of new (optimal) treatment
  ##############################################################################
  
  # Build the formula for the model (X1 is reference treatment)
  new_num_trts = PARAM_ARGS[["NUM_TRTS"]] + 1
  FORMULA = paste0("Y ~", paste0("X", 2:new_num_trts, collapse = "+"))
  
  # Add the optimal treatment, which is now the last one for this set of sims
  BETAS = rep(0, new_num_trts)# Initialize vector of null effects
  BETAS[1] = MODEL_ARGS[["INTERCEPT"]]
  BETAS[new_num_trts] = PARAM_ARGS[["EFFECT_SIZE"]] 
  
  # Initialize a new column for the new treatment to current set of data
  TRIAL_DATA$X4 = 0
  
  # Create data for doing an initialization run of the new treatment
  X = matrix(0, nrow = PARAM_ARGS[["NUM_OBVS_INIT"]], ncol = new_num_trts)
  X[, 1] = X[, new_num_trts] = 1
  ...
  new_data = cbind(X, Y = Y) %>%
    tibble::as_tibble() %>%
    dplyr::mutate(
      period = 21, # spend a treatment period to run this initialization phase
      trt = new_num_trts
    )
  ...
  # Must re-run brms since a new column was added to the data
  POSTERIOR = brms::brm(formula = FORMULA,
                        data = TRIAL_DATA,
                        family = gaussian(link = "identity"),
                        fit = MODEL_ARGS[["COMPILED_FIT2"]],
                        chains = MODEL_ARGS[["NUM_CHAINS"]],
                        warmup = MODEL_ARGS[["NUM_ITER_WARMUP"]],
                        iter = MODEL_ARGS[["NUM_ITER_POST"]],
                        thin = MODEL_ARGS[["THINNING_FACTOR"]],
  ...
  # Calculate allocation probabilities via on Thompson Sampling
  current_probs = TS(POSTERIOR, 
                     c = 21 / (2 * PARAM_ARGS[["MAX_DURATION"]]), 
                     objective = MODEL_ARGS[["OBJECTIVE"]]) 
  
  # Append new set of allocation probabilities to current set
  ALLOC_PROB_TABLE = rbind(ALLOC_PROB_TABLE, c(current_probs, 21))
  
  ##############################################################################
  # Adaptive phase II: optimal treatment added
  ##############################################################################
  
  adaptive_start = 22 # Restarting the next adaptive phase
  adaptive_ph2_periods = adaptive_start:PARAM_ARGS[["MAX_DURATION"]]
  
  for (period in adaptive_ph2_periods) {
    
    # Select the next treatment based on reallocated probabilities
    next_trt = sample(1:new_num_trts, size = 1, prob = current_probs)
    
    # Construct design matrix and "observe" the outcome under this treatment
    X = matrix(0, nrow = PARAM_ARGS[["NUM_OBVS_ADAPTIVE"]], ncol = new_num_trts)
    X[1] = 1        # intercept
    X[next_trt] = 1 # setting next treatment
  ...
  names(ALLOC_PROB_TABLE) = c(paste0("prob_X", 1:new_num_trts), "period")
